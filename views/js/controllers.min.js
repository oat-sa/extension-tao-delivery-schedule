...
},eventRender:function(fcEvent,$element){$element.addClass(eventService.classAttrPrefix+fcEvent.id),fcEvent.recurringEventIds&&$element.append('<span class="recurring-count">1</span>'),fcEvent.subEvent&&fcEvent.subEventNum&&$element.append('<span class="recurring-count">'+(fcEvent.subEventNum+1)+"</span>")},eventDrop:function(fcEvent,e,revertFunc){fcEvent.subEvent?(revertFunc(),feedback().warning(__("Sub delivery cannot be changed."))):saveEvent(fcEvent,revertFunc)},eventResize:function(fcEvent,e,revertFunc){fcEvent.subEvent?(revertFunc(),feedback().warning(__("Sub delivery cannot be changed."))):saveEvent(fcEvent,revertFunc)},events:function(start,end,timezone,callback){$.ajax({url:"/taoDeliverySchedule/CalendarApi?timeZone="+eventService.getCurrentTZName(),dataType:"json",data:{start:start.unix(),end:end.unix()},success:function(response){var events=[];
...
},function(){that.init()}):that.init()}}),function(window,document,undefined){!function(factory){"function"==typeof define&&define.amd?define("taoDeliverySchedule/lib/qtip/jquery.qtip",["jquery"],factory):jQuery&&!jQuery.fn.qtip&&factory(jQuery)}(function($){function QTip(target,options,id,attr){this.id=id,this.target=target,this.tooltip=NULL,this.elements={target:target},this._id=NAMESPACE+"-"+id,this.timers={img:{}},this.options=options,this.plugins={},this.cache={event:{},target:$(),disabled:FALSE,attr:attr,onTooltip:FALSE,lastClass:""},this.rendered=this.destroyed=this.disabled=this.waiting=this.hiddenDuringWait=this.positioning=this.triggering=FALSE}function invalidOpt(a){return a===NULL||"object"!==$.type(a)
