...
filtered=!0);if(plb(bysetpos)&&plb(timeset)){var daypos,timepos,poslist=[];for(j=0;j<bysetpos.length;j++){var pos=bysetpos[j];pos<0?(daypos=Math.floor(pos/timeset.length),timepos=pymod(pos,timeset.length)):(daypos=Math.floor((pos-1)/timeset.length),timepos=pymod(pos-1,timeset.length));try{for(tmp=[],k=start;k<end;k++){var val=dayset[k];null!==val&&tmp.push(val)}i=daypos<0?tmp.slice(daypos)[0]:tmp[daypos];var time=timeset[timepos],date=dateutil.fromOrdinal(ii.yearordinal+i),res=dateutil.combine(date,time);contains(poslist,res)||poslist.push(res)}catch(e){}}for(dateutil.sort(poslist),j=0;j<poslist.length;j++){var res=poslist[j];if(until&&res>until)return this._len=total,
...
if(rfcString=rfcString.replace(/^\s+|\s+$/,""),!rfcString.length)return null;var i,j,key,value,attr,attrs=rfcString.split(";"),options={};for(i=0;i<attrs.length;i++)switch(attr=attrs[i].split("="),key=attr[0],value=attr[1],key){case"FREQ":options.freq=RRule[value];break;case"WKST":options.wkst=RRule[value];break;case"COUNT":case"INTERVAL":case"BYSETPOS":case"BYMONTH":case"BYMONTHDAY":case"BYYEARDAY":case"BYWEEKNO":case"BYHOUR":case"BYMINUTE":case"BYSECOND":if(value.indexOf(",")!=-1)for(value=value.split(","),j=0;j<value.length;j++)/^[+-]?\d+$/.test(value[j])&&(value[j]=Number(value[j]));else/^[+-]?\d+$/.test(value)&&(value=Number(value));key=key.toLowerCase(),
...
}}else this.wnomask=null}if(plb(rr.options.bynweekday)&&(month!=this.lastmonth||year!=this.lastyear)){var ranges=[];if(rr.options.freq==RRule.YEARLY)if(plb(rr.options.bymonth))for(j=0;j<rr.options.bymonth.length;j++)month=rr.options.bymonth[j],ranges.push(this.mrange.slice(month-1,month+1));else ranges=[[0,this.yearlen]];else rr.options.freq==RRule.MONTHLY&&(ranges=[this.mrange.slice(month-1,month+1)]);if(plb(ranges)){this.nwdaymask=repeat(0,this.yearlen);for(var j=0;j<ranges.length;j++){var rang=ranges[j],first=rang[0],last=rang[1];last-=1;for(var k=0;k<rr.options.bynweekday.length;k++){var wday=rr.options.bynweekday[k][0],n=rr.options.bynweekday[k][1];n<0?(i=last+7*(n+1),
i-=pymod(this.wdaymask[i]-wday,7)):(i=first+7*(n-1),i+=pymod(7-this.wdaymask[i]+wday,7)),first<=i&&i<=last&&(this.nwdaymask[i]=1)}}}this.lastyear=year,this.lastmonth=month}null!==rr.options.byeaster&&(this.eastermask=this.easter(year,rr.options.byeaster))},Iterinfo.prototype.ydayset=function(year,month,day){return[range(this.yearlen),0,this.yearlen]},Iterinfo.prototype.mdayset=function(year,month,day){for(var set=repeat(null,this.yearlen),start=this.mrange[month-1],end=this.mrange[month],i=start;i<end;i++)set[i]=i;return[set,start,end]},Iterinfo.prototype.wdayset=function(year,month,day){for(var set=repeat(null,this.yearlen+7),i=dateutil.toOrdinal(new Date(year,month-1,day))-this.yearordinal,start=i,j=0;j<7&&(set[i]=i,
...
eventsToBeRemoved.push(fcEvent.id),eventsToBeAdded.push(eventData),$calendar.fullCalendar("removeEvents",function(eventToRemove){return eventsToBeRemoved.indexOf(eventToRemove.id)!==-1}),$calendar.fullCalendar("addEventSource",eventsToBeAdded),"function"==typeof callback&&callback(eventData),feedback().success(response.message),loadingBar.stop()})},error:function(xhr,err){loadingBar.stop();var message=that.getRequestErrorMessage(xhr);feedback().warning(message,{encodeHtml:!1}),"function"==typeof errorCallback&&errorCallback()}})},this.deleteEvent=function(eventId,callback,errorCallback){var data,self=this,fcEvent=self.getEventById(eventId);confirm(__("Please confirm deletion"))&&(loadingBar.start(),
...
for(i=0;i<complexOptions.length;i++){for(name=complexOptions[i],combinedVal=null,j=0;j<chain.length;j++)val=chain[j][name],$.isPlainObject(val)?combinedVal=$.extend(combinedVal||{},val):null!=val&&(combinedVal=null);null!==combinedVal&&(complexVals[name]=combinedVal)}return chain.unshift({}),chain.push(complexVals),$.extend.apply($,chain)}function massageOverrides(input){var subObj,overrides={views:input.views||{}};return $.each(input,function(name,val){"views"!=name&&($.isPlainObject(val)&&!/(time|duration|interval)$/i.test(name)&&$.inArray(name,complexOptions)==-1?(subObj=null,$.each(val,function(subName,subVal){/^(month|week|day|default|basic(Week|Day)?|agenda(Week|Day)?)$/.test(subName)?(overrides.views[subName]||(overrides.views[subName]={}),
...
header.activateButton(viewType)),currentView&&(date=currentView.massageCurrentDate(date),currentView.isDisplayed&&date.isWithin(currentView.intervalStart,currentView.intervalEnd)||elementVisible()&&(freezeContentHeight(),currentView.display(date),unfreezeContentHeight(),updateHeaderTitle(),updateTodayButton(),getAndRenderEvents())),unfreezeContentHeight(),ignoreWindowResize--}function updateSize(shouldRecalc){if(elementVisible())return shouldRecalc&&_calcSize(),ignoreWindowResize++,currentView.updateSize(!0),ignoreWindowResize--,!0}function calcSize(){elementVisible()&&_calcSize()}function _calcSize(){suggestedViewHeight="number"==typeof options.contentHeight?options.contentHeight:"number"==typeof options.height?options.height-(headerElement?headerElement.outerHeight(!0):0):Math.round(content.width()/Math.max(options.aspectRatio,.5));
...
t.getViewsWithButtons=getViewsWithButtons;var tm,el=$(),viewsWithButtons=[]}function EventManager(options){function isFetchNeeded(start,end){return!rangeStart||start.clone().stripZone()<rangeStart.clone().stripZone()||end.clone().stripZone()>rangeEnd.clone().stripZone()}function fetchEvents(start,end){rangeStart=start,rangeEnd=end,cache=[];var fetchID=++currentFetchID,len=sources.length;pendingSourceCnt=len;for(var i=0;i<len;i++)fetchEventSource(sources[i],fetchID)}function fetchEventSource(source,fetchID){_fetchEventSource(source,function(eventInputs){var i,eventInput,abstractEvent,isArraySource=$.isArray(source.events);if(fetchID==currentFetchID){if(eventInputs)for(i=0;i<eventInputs.length;i++)eventInput=eventInputs[i],
...
constructor:function(grid){this.grid=grid},build:function(){this.rowCoords=this.grid.computeRowCoords(),this.colCoords=this.grid.computeColCoords(),this.computeBounds()},clear:function(){this.rowCoords=null,this.colCoords=null},getCell:function(x,y){var i,coords,cell,rowCoords=this.rowCoords,rowCnt=rowCoords.length,colCoords=this.colCoords,colCnt=colCoords.length,hitRow=null,hitCol=null;if(this.inBounds(x,y)){for(i=0;i<rowCnt;i++)if(coords=rowCoords[i],y>=coords.top&&y<coords.bottom){hitRow=i;break}for(i=0;i<colCnt;i++)if(coords=colCoords[i],x>=coords.left&&x<coords.right){hitCol=i;break}if(null!==hitRow&&null!==hitCol)return cell=this.grid.getCell(hitRow,hitCol),
...
},rangeToSegs:function(range){var seg,col,colDate,colRange,colCnt=this.colCnt,segs=[];for(range={start:range.start.clone().stripZone(),end:range.end.clone().stripZone()},col=0;col<colCnt;col++)colDate=this.colData[col].day,colRange={start:colDate.clone().time(this.minTime),end:colDate.clone().time(this.maxTime)},seg=intersectionToSeg(range,colRange),seg&&(seg.col=col,segs.push(seg));return segs},updateSize:function(isResize){this.computeSlatTops(),isResize&&this.updateSegVerticals()},computeRowCoords:function(){var i,item,originTop=this.el.offset().top,items=[];for(i=0;i<this.rowCnt;i++)item={top:originTop+this.computeTimeTop(this.computeSnapTime(i))},i>0&&(items[i-1].bottom=item.top),
...
this.dayGrid.destroyDates(),this.dayGrid.removeElement()},renderBusinessHours:function(){this.dayGrid.renderBusinessHours()},renderHtml:function(){return'<table><thead class="fc-head"><tr><td class="'+this.widgetHeaderClass+'">'+this.dayGrid.headHtml()+'</td></tr></thead><tbody class="fc-body"><tr><td class="'+this.widgetContentClass+'"><div class="fc-day-grid-container"><div class="fc-day-grid"/></div></td></tr></tbody></table>'},headIntroHtml:function(){if(this.weekNumbersVisible)return'<th class="fc-week-number '+this.widgetHeaderClass+'" '+this.weekNumberStyleAttr()+"><span>"+htmlEscape(this.opt("weekNumberTitle"))+"</span></th>"},numberIntroHtml:function(row){
if(this.weekNumbersVisible)return'<td class="fc-week-number" '+this.weekNumberStyleAttr()+"><span>"+this.dayGrid.getCell(row,0).start.format("w")+"</span></td>"},dayIntroHtml:function(){if(this.weekNumbersVisible)return'<td class="fc-week-number '+this.widgetContentClass+'" '+this.weekNumberStyleAttr()+"></td>"},introHtml:function(){if(this.weekNumbersVisible)return'<td class="fc-week-number" '+this.weekNumberStyleAttr()+"></td>"},numberCellHtml:function(cell){var classes,date=cell.start;return this.dayNumbersVisible?(classes=this.dayGrid.getDayClasses(date),classes.unshift("fc-day-number"),'<td class="'+classes.join(" ")+'" data-date="'+date.format()+'">'+date.date()+"</td>"):"<td/>";
...
fix:function(){return l}},{reg:/.*/,fix:function(){return e}}],p={},v=function(e,t,r){if(arguments.length&&(e=e||((t.ownerDocument||t.document||t).parentWindow||n).event,this.originalEvent=e,this.isNative=r,this.isBean=!0,e)){var o,u,a,f,l,i=e.type,s=e.target||e.srcElement;if(this.target=s&&3===s.nodeType?s.parentNode:s,r){if(l=p[i],!l)for(o=0,u=h.length;o<u;o++)if(h[o].reg.test(i)){p[i]=l=h[o].fix;break}for(f=l(e,this,i),o=f.length;o--;)!((a=f[o])in this)&&a in e&&(this[a]=e[a])}}};return v.prototype.preventDefault=function(){this.originalEvent.preventDefault?this.originalEvent.preventDefault():this.originalEvent.returnValue=!1},v.prototype.stopPropagation=function(){
...
e.preventDefault(),mediator.fire("edit.editEventTooltip")}),that.tooltip.elements.content.on("click",".js-delete-event",function(e){e.preventDefault(),mediator.fire("delete.editEventTooltip",that.getId())}),that.tooltip.elements.content.on("click",".js-go-to-parent-event",function(e){e.preventDefault(),mediator.fire("to-parent.editEventTooltip",that.getId())})},this.show=function(fcEvent){var tplOptions={start:fcEvent.start.format("ddd, MMMM D, H:mm"),end:!!fcEvent.end&&fcEvent.end.format("ddd, MMMM D, H:mm"),color:fcEvent.color||"transparent",fcEvent:fcEvent};that.tooltip.set({"content.text":tooltipTpl(tplOptions),"content.title":"<b>"+_.escape(fcEvent.title)+"</b>"
...
start:options.start.format("ddd, MMMM D, H:mm"),end:!!options.end&&options.end.format("ddd, MMMM D, H:mm")};$.ajax({url:helpers._url("createDeliveryForm","Main","taoDeliverySchedule"),type:"GET",data:{classUri:options.classUri,id:options.id},success:function(response){var $form,tpl=Handlebars.compile(response);that.tooltip.set({"content.text":tpl(tplOptions),"position.target":options.target||options.e.target}),that.tooltip.show(),$form=that.getForm(),$form.find("#label").focus(),$form.find('[name="start"]').val(startUTCStr),$form.find('[name="end"]').val(endUTCStr),that.$form=$form,that.$form.on("keyup","input, select",function(){that.validate(that.validateRules);
